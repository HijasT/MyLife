    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button
            ...
            onClick={() => setShowDeleteConfirm("__item__")}
          >
            Delete asset
          </button>
          <button
      ...
      <div style={{ maxWidth: 860, margin: "0 auto", padding: "24px 20px" }}>
        ...
        >
          {[
            {
              label: `Total ${item.unitLabel}s`,
              value: fmtNum(stats.totalUnits, 4),
              color: V.accent,
            },
            {
              label: "Cost basis",
              value: `AED ${fmtNum(stats.costBasisAed)}`,
              color: V.text,
            },
            {
              label: "Bought",
              value: `AED ${fmtNum(stats.totalBuysAed)}`,
              color: V.text,
            },
            {
              label: "Sold",
              value: `AED ${fmtNum(stats.totalSellsAed)}`,
              color: V.text,
            },
            {
              label: "Current value",
              value:
                stats.currentValueAed !== null
                  ? `AED ${fmtNum(stats.currentValueAed)}`
                  : "No price",
              color: V.text,
            },
            {
              label: "Unrealized P&L",
              value: stats.pl !== null ? fmtSignedAed(stats.pl) : "—",
              color: stats.pl !== null ? plColor : V.text,
            },
            {
              label: "Unrealized P&L %",
              value:
                stats.plPct !== null
                  ...
                  : "—",
              color: stats.plPct !== null ? plColor : V.text,
            },
            {
              label: "Realized P&L",
              value: fmtSignedAed(stats.realizedPlAed),
              color: stats.realizedPlAed >= 0 ? "#16a34a" : "#ef4444",
            },
            {
              label: `Avg cost/${item.unitLabel}`,
              value: `AED ${fmtNum(stats.avgUnitPrice)}`,
              color: V.muted,
            },
          ].map((s) => (
            <div
        ...
        <div style={section}>
          ...
          >
            {item.currentPrice ? (
              <>
                <span style={{ fontSize: 20, fontWeight: 800, color: V.accent }}>
          ...
          {transactionRows.map((row, idx) => {
            ...
            return (
              <div
                ...
                style={{
                  padding: "13px 16px",
                  borderBottom: idx < transactionRows.length - 1 ? `1px solid ${V.border}` : "none",
                }}
              >
                <div
                  style={{
                ...
                >
                  <div>
                    ...
                    >
                      <span
                        style={{
                          ...
                          textTransform: "uppercase",
                          letterSpacing: "0.08em",
                          color: p.transactionType === "buy" ? "#16a34a" : "#ef4444",
                          border: `1px solid ${
                      ...
                      {p.source && (
                        <span style={{ fontSize: 11, color: V.faint }}>via {p.source}</span>
                      )}
                    </div>
                    ...
                    <div style={{ fontSize: 12, color: V.muted, marginTop: 3 }}>
                      ...
                      <strong style={{ color: V.text }}>
                        {p.currency} {fmtNum(p.unitPrice)}
                      </strong>
                      {p.notes && (
                        <span style={{ fontStyle: "italic", marginLeft: 10 }}>{p.notes}</span>
                      )}
                    </div>
                  </div>
                  ...
                  >
                    <div style={{ fontSize: 14, fontWeight: 800 }}>
                      {p.transactionType === "sell" ? "Received" : "Paid"}: {p.currency}{" "}
                      {fmtNum(p.totalPaid)}
                    </div>
          ...
          >
            ...
            >
              <div>
                ...
                <div style={{ fontSize: 18, fontWeight: 800 }}>
                  {editPurchase ? "Edit transaction" : "Add transaction"}
                </div>
              </div>
              <button style={btn} onClick={() => setShowAdd(false)}>
                ✕
              </button>
            </div>
            ...
            >
              <label style={{ ...lbl, gridColumn: "1/-1" }}>
                ...
                <select
                  style={inp}
                  value={af.transactionType}
                  onChange={(e) =>
                    setAf((p) => ({ ...p, transactionType: e.target.value as TxType }))
                  }
                >
              ...
              <label style={{ ...lbl, gridColumn: "1/-1" }}>
                ...
                <input
                  type="datetime-local"
                  style={inp}
                  value={af.purchasedAt}
                  onChange={(e) => setAf((p) => ({ ...p, purchasedAt: e.target.value }))}
                />
              </label>
              ...
              <label style={lbl}>
                ...
                <input
                  ...
                  style={inp}
                  value={af.unitPrice}
                  onChange={(e) => setAf((p) => ({ ...p, unitPrice: e.target.value }))}
              ...
              <label style={lbl}>
                ...
                <input
                  ...
                  placeholder="e.g. 1.069"
                />
              </label>

              <label style={lbl}>
                Total {af.transactionType === "sell" ? "received" : "paid"} (actual amount)
                <input
                  type="number"
                  style={inp}
                  value={af.totalPaid}
                  onChange={(e) => setAf((p) => ({ ...p, totalPaid: e.target.value }))}
              ...
              <label style={lbl}>
                ...
                <select
                  ...
                  value={af.currency}
                  onChange={(e) =>
                    setAf((p) => ({ ...p, currency: e.target.value as Currency }))
                  }
                >
                  <option>AED</option>
                  <option>USD</option>
              ...
              <label style={{ ...lbl, gridColumn: "1/-1" }}>
                ...
                <input
                  style={inp}
                  value={af.source}
                  onChange={(e) => setAf((p) => ({ ...p, source: e.target.value }))}
                  placeholder="e.g. ENBD, Kitco, Binance"
                />
              </label>
              ...
              <label style={{ ...lbl, gridColumn: "1/-1" }}>
                ...
                <input
                  style={inp}
                  value={af.notes}
                  onChange={(e) => setAf((p) => ({ ...p, notes: e.target.value }))}
                />
              </label>
            </div>
      ...
      {showUpdatePrice && (
        ...
        >
          ...
          >
            ...
            >
              <button style={btn} onClick={() => setShowUpdatePrice(false)}>
                Cancel
              </button>
              <button style={btnPrimary} onClick={updatePrice}>
                Update
              </button>
            </div>
          </div>
          ...
          >
            ...
            <div style={{ fontSize: 13, color: V.muted, marginBottom: 16 }}>
              {showDeleteConfirm === "__item__"
                ? "This will delete the asset and all its purchases. Cannot be undone."
                : "This cannot be undone."}
            </div>

            <div style={{ display: "flex", justifyContent: "flex-end", gap: 8 }}>
              <button style={btn} onClick={() => setShowDeleteConfirm(null)}>
                Cancel
              </button>
              <button
