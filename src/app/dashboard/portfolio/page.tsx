...
export default function PortfolioPage() {
  ...
  const [livePrices, setLivePrices] = useState<
    Record<string, { bid: number; ask: number; updated: string }>
  >({});
  ...
  useEffect(() => {
    async function load() {
      ...
      if (profileRes.data?.goldapi_key) {
        setGoldApiKey(profileRes.data.goldapi_key);
        setGoldApiInput(profileRes.data.goldapi_key);
      }
  ...
  async function fetchSpotPrices() {
    ...
    if (Object.keys(results).length > 0) {
      ...
      showToast("⚠ No prices — Yahoo Finance may be rate-limiting, try again in 30s");
    }
  }
  ...
  async function addItem() {
    ...
    const notesVal = newItem.livePriceLink
      ? `liveprice:${newItem.livePriceLink}||${newItem.notes}`
      : newItem.notes;
              ...
              <input
                ...
                onKeyDown={(e) => {
                  ...
                  if (
                    e.key === "Enter" &&
                    trimmed &&
                    !customSymbols.includes(trimmed)
                  ) {
                      ...
                      >
                        ...
                        <div style={{ textAlign: "right" }}>
                          ...
                          <div style={{ fontSize: 11, color: V.faint, marginTop: 2 }}>
                            Cost basis: AED {fmtN(s.costBasisAed)}
                          </div>
          ...
          >
            ...
            >
              ...
              <label style={{ ...lbl, gridColumn: "1/-1" }}>
                ...
                <select
                  style={inp}
                  value={newItem.livePriceLink}
                  onChange={(e) =>
                    setNewItem((p) => ({ ...p, livePriceLink: e.target.value }))
                  }
                >
                ...
                <span style={{ fontSize: 11, color: V.faint, marginTop: 2 }}>
                  Asset current price will auto-update from live prices tab (Bid/Sell
                  rate)
                </span>
              </label>
