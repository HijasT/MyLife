...
export default function PerfumeDetailPage({ params }: { params: { id: string } }) {
  ...
  const [newBottle, setNewBottle] = useState({ bottleType: "Full bottle" as BottleType, sizeMl: "100", price: "", date: nowDubai().slice(0,10), shopName: "", shopLink: "" });
  ...
  async function update(partial: Partial<Perfume>) {
    ...
    if (partial.targetPriceAed !== undefined) db.target_price_aed = partial.targetPriceAed;
    if (partial.preferredShop !== undefined) db.preferred_shop = partial.preferredShop;
  ...
  async function addBottle() {
    ...
    const { data: bd, error: bottleErr } = await supabase.from("perfume_bottles").insert({
      perfume_id: item.id, user_id: userId, bottle_size_ml: size, bottle_type: newBottle.bottleType, status: "In collection", usage: "Casual",
    }).select("*").single();
    ...
    setNewBottle({ bottleType: "Full bottle", sizeMl: "100", price: "", date: nowDubai().slice(0,10), shopName: "", shopLink: "" });
  ...
  async function copyToWishlist() {
    ...
    const { data, error } = await supabase.from("perfumes").insert({
      ...
      clone_similar: item.cloneSimilar, notes_text: item.notesText, purchase_priority: item.purchasePriority ?? "Medium", target_price_aed: item.targetPriceAed ?? null, preferred_shop: item.preferredShop ?? null,
    }).select("*").single();
    ...
    <div style={{ background: V.bg, minHeight: "100vh", color: V.text, fontFamily: "system-ui,sans-serif" }}>
      <div style={{ position: "sticky", top: 0, zIndex: 20, background: isDark ? "rgba(13,15,20,0.9)" : "rgba(249,248,245,0.9)", backdropFilter: "blur(12px)", borderBottom: `1px solid ${V.border}`, padding: "12px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          ...
          <a href={`https://www.fragrantica.com/search/?query=${encodeURIComponent(`${item.brand} ${item.model}`)}`} target="_blank" rel="noreferrer" style={{ ...btnStyle, textDecoration: "none" }}>Fragrantica search</a>
      ...
      <div style={{ maxWidth: 920, margin: "0 auto", padding: "28px 20px" }}>
        ...
        <div style={sectionStyle}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            <span style={labelStyle}>Wishlist budget & buy plan</span>
            {item.status === "wishlist" && <span style={{ fontSize: 12, color: V.muted }}>Priority {item.purchasePriority || "Medium"}</span>}
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 10 }}>
            ...
            <label><span style={labelStyle}>Target price (AED)</span><input style={inputStyle} value={item.targetPriceAed ?? ""} onChange={e => update({ targetPriceAed: safeNum(e.target.value, 0) || null })} /></label>
            <label><span style={labelStyle}>Preferred shop</span><input style={inputStyle} value={item.preferredShop ?? ""} onChange={e => update({ preferredShop: e.target.value })} /></label>
          </div>
        </div>
        ...
        <div style={sectionStyle}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            ...
            {item.cloneSimilar && <span style={{ fontSize: 12, color: V.muted }}>Clone note: {item.cloneSimilar}</span>}
          </div>
        ...
        <div style={sectionStyle}>
          <span style={labelStyle}>Collection notes</span>
          <textarea style={{ ...inputStyle, minHeight: 100 }} value={item.notesText} onChange={e => setItem(prev => prev ? { ...prev, notesText: e.target.value } : prev)} onBlur={e => update({ notesText: e.target.value })} />
        </div>
        ...
        <div style={sectionStyle}>
          ...
          {item.bottles.length === 0 ? <span style={{ fontSize: 13, color: V.faint }}>No purchases recorded.</span> : <div style={{ display: "grid", gap: 8 }}>{item.bottles.map(b => {
            ...
            return <div key={b.id} style={{ display: "grid", gridTemplateColumns: "1fr auto auto auto", gap: 10, alignItems: "center", padding: "10px 12px", border: `1px solid ${V.border}`, borderRadius: 10, background: V.inputBg }}>
