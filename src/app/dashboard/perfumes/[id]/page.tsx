...
type Bottle = { id: string; bottleSizeMl: number; bottleType: BottleType; status: BottleStatus; occasion: string[] };
...
function dbToItem(row: any): Perfume {
  return {
    ...
    occasionTags: String(row.usage_type ?? "").split(",").map((x:string)=>x.trim()).filter(Boolean),
    ...
    bottles: (row.perfume_bottles ?? []).map((b: any): Bottle => ({
      ...
      occasion: String(b.usage ?? "").split(",").map((x:string)=>x.trim()).filter(Boolean),
    })),
...
export default function PerfumeDetailPage({ params }: { params: { id: string } }) {
  ...
  const [newBottle, setNewBottle] = useState({ bottleType: "Bottle" as BottleType, sizeMl: "100", price: "", date: nowDubai().slice(0,10), shopName: "", shopLink: "" });
  const [wearForm, setWearForm] = useState({ wornOn: todayDubai(), occasion: "Casual", sprays: "6", weatherTag: "Neutral", compliment: false, performance: "Moderate" });
  ...
  async function updateBottle(bottleId: string, partial: Partial<Bottle>) {
    ...
    if (partial.occasion !== undefined) db.usage = partial.occasion.join(", ");
  ...
  async function removeBottle(bottleId: string, purchaseId?: string) {
    if (!confirm("Remove this bottle and its purchase record?")) return;
    if (purchaseId) {
      const { error: pe } = await supabase.from("perfume_purchases").delete().eq("id", purchaseId);
      if (pe) { showToast(pe.message); return; }
  ...
  async function archiveBottle(bottleId: string) {
    const reason = prompt("Archive reason: sold, emptied, or gifted", "emptied");
    if (!reason) return;
    const normalized = reason.trim().toLowerCase();
    const map = normalized === "sold" ? "Sold" : normalized === "gifted" ? "Gifted" : "Emptied";
    if (map === "Sold") {
      prompt("Resale price in AED", "0");
    }
    await updateBottle(bottleId, { status: map as BottleStatus });
    showToast(`Bottle archived as ${map}`);
  }
  ...
  async function addBottle() {
    ...
    const { data: bd, error: bottleErr } = await supabase.from("perfume_bottles").insert({
      perfume_id: item.id, user_id: userId, bottle_size_ml: size, bottle_type: (newBottle.bottleType === "Bottle" ? "Full bottle" : newBottle.bottleType), status: "In collection", usage: null,
    }).select("*").single();
    ...
    setItem(prev => prev ? { ...prev, bottles: [...prev.bottles, { id: bd.id, bottleSizeMl: bd.bottle_size_ml, bottleType: (bd.bottle_type === "Full bottle" ? "Bottle" : bd.bottle_type), status: (bd.status === "In collection" ? "Wardrobe" : bd.status), occasion: [] }] } : prev);
    setShowAddBottle(false);
    setNewBottle({ bottleType: "Bottle", sizeMl: "100", price: "", date: nowDubai().slice(0,10), shopName: "", shopLink: "" });
  ...
  async function copyToWishlist() {
    ...
    const { data, error } = await supabase.from("perfumes").insert({
      ...
      notes_tags: item.notesTags, weather_tags: item.weatherTags, gender_scale: item.genderScale, longevity: item.longevity, sillage: item.sillage, value_rating: item.value, usage_type: item.occasionTags.join(", "),
  ...
  const priceStats = useMemo(() => {
    ...
    return { totalSpent, price100ml, bestDeal, purchaseCount: purchases.length };
  }, [purchases]);
  ...
  return (
    <div style={{ background: V.bg, minHeight: "100vh", color: V.text, fontFamily: "system-ui,sans-serif" }}>
      ...
      <div style={{ maxWidth: 920, margin: "0 auto", padding: "28px 20px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "240px 1fr", gap: 24, marginBottom: 28, alignItems: "start" }}>
          <div>
            ...
            <button style={{ ...btnStyle, width: "100%", marginTop: 10 }} onClick={() => { setPhotoInput(item.imageUrl || `https://www.google.com/search?tbm=isch&q=${encodeURIComponent(`${item.brand} ${item.model} perfume`)}`); setPhotoMode("url"); setShowPhoto(true); }}>📷 Change photo</button>
          </div>
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(130px,1fr))", gap: 10 }}>
              ...
              {[ ["Longevity", item.longevity], ["Projection", item.sillage], ["Gender", genderLabel(item.genderScale)], ["Performance", item.value], ["Occasion", item.occasionTags.join(", ")] ].map(([k,v]) => <div key={String(k)} style={{ background: V.inputBg, borderRadius: 10, padding: "10px 12px", border: `1px solid ${V.border}` }}><span style={labelStyle}>{k}</span><span style={{ fontSize: 13, fontWeight: 700 }}>{v || "—"}</span></div>)}
            </div>
            {isEdit && <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 10, marginTop: 12 }}>
              <label><span style={labelStyle}>Longevity</span><select style={inputStyle} value={item.longevity || "Moderate"} onChange={e => update({ longevity: e.target.value })}><option>Weak</option><option>Moderate</option><option>Strong</option><option>Beast</option></select></label>
              <label><span style={labelStyle}>Projection</span><select style={inputStyle} value={item.sillage || "Moderate"} onChange={e => update({ sillage: e.target.value })}><option>Soft</option><option>Moderate</option><option>Strong</option><option>Loud</option></select></label>
              <label><span style={labelStyle}>Gender</span><select style={inputStyle} value={String(item.genderScale)} onChange={e => update({ genderScale: Number(e.target.value) as GenderScale })}><option value="0">Masculine</option><option value="1">Lean masc.</option><option value="2">Unisex</option><option value="3">Lean fem.</option><option value="4">Feminine</option></select></label>
              <label><span style={labelStyle}>Performance</span><select style={inputStyle} value={item.value || "Neutral"} onChange={e => update({ value: e.target.value as Perfume["value"] })}><option>Worth it</option><option>Neutral</option><option>Not worth it</option></select></label>
              <label style={{ gridColumn: "1/-1" }}><span style={labelStyle}>Occasion</span><select multiple style={{ ...inputStyle, minHeight: 110 }} value={item.occasionTags} onChange={e => update({ occasionTags: Array.from((e.target as HTMLSelectElement).selectedOptions).map(o => o.value) })}><option>Casual</option><option>Office</option><option>Party</option><option>Date night</option><option>Gym</option><option>Travel</option><option>Wedding</option><option>Summer day</option><option>Winter evening</option></select></label>
            </div>}
          </div>
        </div>
        ...
        <div style={sectionStyle}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            ...
            <span style={{ fontSize: 12, color: V.muted }}>Manual entry only</span>
          </div>
          <div style={{ display: "grid", gap: 10 }}><label><span style={labelStyle}>Clone / similar</span><input style={inputStyle} value={item.cloneSimilar} onChange={e => update({ cloneSimilar: e.target.value })} placeholder="Manual note e.g. Similar to CDNIM / Tygar style" /></label>{similarItems.length === 0 ? <div style={{ fontSize: 13, color: V.faint }}>No obvious overlap found yet.</div> : <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 10 }}>{similarItems.map(s => <button key={s.id} onClick={() => router.push(`/dashboard/perfumes/${s.id}`)} style={{ textAlign: "left", border: `1px solid ${V.border}`, background: V.inputBg, borderRadius: 12, padding: 12, cursor: "pointer" }}><div style={{ fontSize: 12, color: V.faint, textTransform: "uppercase", fontWeight: 800 }}>{s.brand}</div><div style={{ fontSize: 14, fontWeight: 800 }}>{s.model}</div><div style={{ fontSize: 12, color: V.muted }}>Similarity score {s.score.toFixed(1)}</div></button>)}</div>}</div>
        </div>
        ...
        <div style={sectionStyle}>
          ...
          {item.bottles.length === 0 ? <span style={{ fontSize: 13, color: V.faint }}>No purchases recorded.</span> : <div style={{ display: "grid", gap: 10 }}>{item.bottles.map(b => {
            const pur = purchases.find(p => p.bottleId === b.id);
            const archived = b.status !== "Wardrobe";
            return <div key={b.id} style={{ padding: "12px", border: `1px solid ${V.border}`, borderRadius: 12, background: V.inputBg }}>
              <div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "center", marginBottom: 10 }}>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
                  <span style={{ fontWeight: 800 }}>{b.bottleType}</span>
                  <span style={{ color: V.muted }}>{b.bottleSizeMl} ml</span>
                  <span style={{ padding: "4px 8px", borderRadius: 999, background: archived ? "rgba(107,114,128,0.12)" : "rgba(245,166,35,0.12)", color: archived ? V.muted : "#d97706", fontSize: 11, fontWeight: 700 }}>{archived ? `Archive · ${b.status}` : "Wardrobe"}</span>
                </div>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  {!archived && <button style={btnStyle} onClick={() => archiveBottle(b.id)}>Archive</button>}
                  {archived && <button style={btnStyle} onClick={() => updateBottle(b.id, { status: "Wardrobe" })}>Move to wardrobe</button>}
                  <button style={dangerBtnStyle} onClick={() => removeBottle(b.id, pur?.id)}>Remove</button>
                </div>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(150px,1fr))", gap: 10 }}>
                <label><span style={labelStyle}>Bottle type</span><select style={inputStyle} value={b.bottleType} onChange={e => updateBottle(b.id, { bottleType: e.target.value as BottleType })}><option>Bottle</option><option>Decant</option><option>Sample</option></select></label>
                <label><span style={labelStyle}>Size (ml)</span><input style={inputStyle} type="number" value={b.bottleSizeMl} onChange={e => updateBottle(b.id, { bottleSizeMl: safeNum(e.target.value, b.bottleSizeMl) })} /></label>
                <label><span style={labelStyle}>Price (AED)</span><input style={inputStyle} type="number" value={pur?.price ?? 0} onChange={e => pur && updatePurchase(pur.id, { price: safeNum(e.target.value, 0) })} /></label>
                <label><span style={labelStyle}>Purchase date</span><input style={inputStyle} type="date" value={pur?.date ?? ""} onChange={e => pur && updatePurchase(pur.id, { date: e.target.value })} /></label>
                <label><span style={labelStyle}>Shop name</span><input style={inputStyle} value={pur?.shopName ?? ""} onChange={e => pur && updatePurchase(pur.id, { shopName: e.target.value })} /></label>
                <label><span style={labelStyle}>Shop link</span><input style={inputStyle} value={pur?.shopLink ?? ""} onChange={e => pur && updatePurchase(pur.id, { shopLink: e.target.value })} placeholder="https://…" /></label>
              </div>
            </div>;
          })}</div>}
        </div>
      ...
      {showAddBottle && <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 100, display: "grid", placeItems: "center", padding: 16 }}><div style={{ background: V.card, border: `1px solid ${V.border}`, borderRadius: 16, width: "min(620px,100%)" }}><div style={{ padding: "18px 20px", borderBottom: `1px solid ${V.border}` }}><div style={{ fontSize: 18, fontWeight: 800 }}>Add bottle</div><div style={{ fontSize: 12, color: V.muted }}>One purchase creates one bottle. Bottle and purchase stay editable after saving.</div></div><div style={{ padding: 20, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}><label><span style={labelStyle}>Bottle type</span><select style={inputStyle} value={newBottle.bottleType} onChange={e => setNewBottle(p => ({ ...p, bottleType: e.target.value as BottleType }))}><option>Bottle</option><option>Decant</option><option>Sample</option></select></label><label><span style={labelStyle}>Size (ml)</span><input style={inputStyle} value={newBottle.sizeMl} onChange={e => setNewBottle(p => ({ ...p, sizeMl: e.target.value }))} /></label><label><span style={labelStyle}>Bottle price (AED)</span><input style={inputStyle} type="number" value={newBottle.price} onChange={e => setNewBottle(p => ({ ...p, price: e.target.value }))} placeholder="0" /></label><label><span style={labelStyle}>Purchase date</span><input style={inputStyle} type="date" value={newBottle.date} onChange={e => setNewBottle(p => ({ ...p, date: e.target.value }))} /></label><label><span style={labelStyle}>Shop name</span><input style={inputStyle} value={newBottle.shopName} onChange={e => setNewBottle(p => ({ ...p, shopName: e.target.value }))} placeholder="Optional" /></label><label style={{ gridColumn: "1/-1" }}><span style={labelStyle}>Shop link</span><input style={inputStyle} value={newBottle.shopLink} onChange={e => setNewBottle(p => ({ ...p, shopLink: e.target.value }))} placeholder="https://…" /></label></div><div style={{ padding: "0 20px 20px", display: "flex", justifyContent: "flex-end", gap: 8 }}><button style={btnStyle} onClick={() => setShowAddBottle(false)}>Cancel</button><button style={primaryBtnStyle} onClick={addBottle}>Add</button></div></div></div>}
      {showWearModal && <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 100, display: "grid", placeItems: "center", padding: 16 }}><div style={{ background: V.card, border: `1px solid ${V.border}`, borderRadius: 18, padding: 20, width: "min(520px,100%)" }}><div style={{ fontSize: 18, fontWeight: 800, marginBottom: 12 }}>Log a wear</div><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}><label><span style={labelStyle}>Date</span><input style={inputStyle} type="date" value={wearForm.wornOn} onChange={e => setWearForm(f => ({ ...f, wornOn: e.target.value }))} /></label><label><span style={labelStyle}>Occasion</span><select style={inputStyle} value={wearForm.occasion} onChange={e => setWearForm(f => ({ ...f, occasion: e.target.value }))}><option>Casual</option><option>Office</option><option>Party</option><option>Date night</option><option>Gym</option><option>Travel</option><option>Wedding</option></select></label><label><span style={labelStyle}>Sprays</span><input style={inputStyle} value={wearForm.sprays} onChange={e => setWearForm(f => ({ ...f, sprays: e.target.value }))} /></label><label><span style={labelStyle}>Weather</span><select style={inputStyle} value={wearForm.weatherTag} onChange={e => setWearForm(f => ({ ...f, weatherTag: e.target.value }))}><option>Hot</option><option>Neutral</option><option>Cold</option><option>Rainy</option><option>Indoor AC</option></select></label><label style={{ gridColumn: "1/-1" }}><span style={labelStyle}>Performance</span><select style={inputStyle} value={wearForm.performance} onChange={e => setWearForm(f => ({ ...f, performance: e.target.value }))}><option>Weak</option><option>Moderate</option><option>Strong</option><option>Excellent</option></select></label><label style={{ display: "flex", alignItems: "center", gap: 8, gridColumn: "1/-1", fontSize: 13 }}><input type="checkbox" checked={wearForm.compliment} onChange={e => setWearForm(f => ({ ...f, compliment: e.target.checked }))} /> Got a compliment</label></div><div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 14 }}><button style={btnStyle} onClick={() => setShowWearModal(false)}>Cancel</button><button style={primaryBtnStyle} onClick={async () => { if (!userId || !item) return; const { data, error } = await supabase.from("perfume_wear_logs").insert({ user_id: userId, perfume_id: item.id, worn_on: wearForm.wornOn, occasion: wearForm.occasion || null, sprays: safeNum(wearForm.sprays, 0), weather_tag: wearForm.weatherTag || null, compliment: wearForm.compliment, performance: wearForm.performance || null }).select("*").single(); if (error || !data) { showToast(error?.message || "Wear log failed"); return; } setWearLogs(prev => [{ id: data.id, wornOn: data.worn_on, occasion: data.occasion ?? "", sprays: data.sprays ?? 0, weatherTag: data.weather_tag ?? "", compliment: !!data.compliment, performance: data.performance ?? "" }, ...prev]); setShowWearModal(false); showToast("Wear logged"); }}>Save</button></div></div></div>}
