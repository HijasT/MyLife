    ...
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <section className="mb-10">
        ...
        <div className="bg-sidebar rounded-2xl p-6 mb-4 flex items-center justify-between overflow-hidden relative">
          <div className="absolute right-0 top-0 bottom-0 w-48 opacity-5"><div className="absolute inset-0 bg-gradient-to-l from-accent" /></div>
          ...
          <div className="relative z-10 flex gap-2">
            {FINANCE_MODULES.map((m) => <div key={m.id} className="w-10 h-10 rounded-xl bg-sidebar-hover flex items-center justify-center text-lg">{m.icon}</div>)}
          </div>
        </div>
      ...
      <section>
        ...
        <div className="rounded-2xl border overflow-hidden" style={{ background:"var(--card-bg)", borderColor:"var(--card-border)" }}>
          {[
            ...
            { label:"Perfume Collection (Aromatica)", phase:2, done:true },
            { label:"Expense Tracker", phase:3, done:false },
            { label:"Budget", phase:4, done:false },
            { label:"Portfolio", phase:5, done:false },
          ].map((item, i) => (
            <div key={item.label} className={`flex items-center gap-4 px-6 py-4 ${i<4?"border-b":""}`} style={{ borderColor:"var(--divider)" }}>
              <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${item.done?"bg-accent text-white":""}`}
                style={!item.done ? { background:"var(--main-bg2)", color:"var(--text-muted)" } : {}}>
                {item.done ? "✓" : item.phase}
              </div>
              <span className={`text-sm flex-1 ${item.done?"font-medium":""}`} style={{ color: item.done?"var(--text-primary)":"var(--text-muted)" }}>
                {item.label}
              </span>
              {item.done && <span className="text-[10px] font-semibold text-accent uppercase tracking-widest">Done</span>}
            </div>
          ))}
        </div>
      </section>
