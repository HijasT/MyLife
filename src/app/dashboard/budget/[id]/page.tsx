...
function monthsBetween(from: string, to: string): string[] {
  const result: string[] = [];
  let cur = from;
  while (cur <= to) { result.push(cur); cur = addMonths(cur, 1); }
  return result;
}
...
export default function DueItemDetailPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (itemRes.data) {
        ...
        setItem({ id:r.id, name:r.name, group:r.group_name??"General", dueDay:r.due_date_day??r.due_day??null, statementDay:r.statement_date??null, defaultCurrency:(r.default_currency??"AED") as Currency, defaultAmount:r.default_amount??null, isFixed:r.is_fixed??false });
      }
      if (entriesRes.data) {
        setEntries(entriesRes.data.map((e: {id:string;month:string;amount:number|null;currency:string;status:string;paid_at:string|null;note:string}) => ({
          id:e.id, month:e.month, amount:e.amount??null, currency:(e.currency??"AED") as Currency,
          status:(e.status??"pending") as Status, paidAt:e.paid_at??null, note:e.note??""
        })));
      }
  ...
  async function addMissingMonth() {
    ...
    const existing = entries.find(e => e.month === newMonth);
    if (existing) { showToast("Month already exists"); return; }
    const amount = newAmount === "" ? item.defaultAmount : parseFloat(newAmount);
    const { data } = await supabase.from("due_entries").insert({
      user_id:userId, due_item_id:item.id, month:newMonth,
      amount, currency:newCurrency, status:"pending", note:newNote
    }).select("*").single();
    if (data) {
      const entry: DueEntry = { id:data.id, month:data.month, amount:data.amount??null, currency:(data.currency??"AED") as Currency, status:(data.status??"pending") as Status, paidAt:data.paid_at??null, note:data.note??"" };
      setEntries(p => [entry, ...p].sort((a,b) => b.month.localeCompare(a.month)));
      setShowAddMonth(false);
      setNewMonth(nowMonth()); setNewAmount(""); setNewNote("");
  ...
  async function togglePaid(entry: DueEntry) {
    ...
    showToast(newStatus === "paid" ? "✓ Paid" : "Unmarked");
  }

  // Missing months calculation
  ...
  const allMonths = monthsBetween(firstMonth, nowMonth());
  const missingMonths = allMonths.filter(m => !existingMonths.has(m));

  const totalPaidAed = entries.filter(e => e.status === "paid").reduce((s, e) => s + toAed(e.amount??0, e.currency, fxRates), 0);
  ...
  return (
    <div style={{ minHeight:"100vh", background:V.bg, color:V.text, fontFamily:"system-ui,sans-serif" }}>
      ...
      <div style={{ position:"sticky", top:0, zIndex:20, background:isDark?"rgba(13,15,20,0.9)":"rgba(249,248,245,0.9)", backdropFilter:"blur(12px)", borderBottom:`1px solid ${V.border}`, padding:"12px 24px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        ...
        <button style={btnPrimary} onClick={() => { setNewCurrency(item.defaultCurrency); setShowAddMonth(true); }}>+ Add month</button>
      </div>
      ...
      <div style={{ maxWidth:800, margin:"0 auto", padding:"24px 20px" }}>
        ...
        <div style={{ marginBottom:20 }}>
          ...
          <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
            {item.statementDay && <span style={{ fontSize:12, fontWeight:700, padding:"3px 10px", borderRadius:999, background:"rgba(245,166,35,0.12)", color:"#F5A623" }}>S: {item.statementDay}th</span>}
            {item.dueDay && <span style={{ fontSize:12, fontWeight:700, padding:"3px 10px", borderRadius:999, background:"rgba(239,68,68,0.1)", color:"#ef4444" }}>D: {item.dueDay}th</span>}
            {item.isFixed && <span style={{ fontSize:12, fontWeight:700, padding:"3px 10px", borderRadius:999, background:"rgba(99,102,241,0.1)", color:"#6366f1" }}>Fixed</span>}
          </div>
        </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(150px,1fr))", gap:10, marginBottom:20 }}>
          {[
            { label:"Total paid", value:`AED ${totalPaidAed.toFixed(0)}`, color:V.accent },
            { label:"Months paid", value:paidCount, color:"#16a34a" },
            { label:"Total records", value:entries.length, color:V.faint },
            { label:"Missing months", value:missingMonths.length, color:missingMonths.length>0?"#ef4444":V.faint },
          ].map(s => (
        ...
        {missingMonths.length > 0 && (
          <div style={{ marginBottom:16, padding:"12px 16px", background:"rgba(239,68,68,0.08)", border:"1px solid rgba(239,68,68,0.2)", borderRadius:12, display:"flex", justifyContent:"space-between", alignItems:"center", gap:12, flexWrap:"wrap" }}>
            <div>
              <div style={{ fontSize:13, fontWeight:700, color:"#ef4444" }}>Missing {missingMonths.length} month{missingMonths.length>1?"s":""}</div>
              <div style={{ fontSize:11, color:V.faint, marginTop:2 }}>{missingMonths.slice(0,4).map(fmtMonth).join(", ")}{missingMonths.length>4?` +${missingMonths.length-4} more`:""}</div>
            </div>
            <button style={{ ...btnPrimary, background:"#ef4444" }} onClick={() => { setNewCurrency(item.defaultCurrency); setNewMonth(missingMonths[0]); setShowAddMonth(true); }}>Add missing</button>
          </div>
        )}
                ...
                {isEditing ? (
                  <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                    ...
                    <div style={{ display:"flex", gap:8 }}>
                      <button style={btnPrimary} onClick={() => saveEdit(entry)}>Save</button>
                    ...
                    <div style={{ display:"flex", gap:10, alignItems:"center" }}>
                      <div style={{ textAlign:"right" }}>
                        <div style={{ fontSize:14, fontWeight:700, color:entry.amount===0?"#16a34a":V.text }}>
                          {entry.amount===0 ? "✓ 0 (paid)" : `${entry.currency} ${entry.amount?.toLocaleString()??""}`}
                        </div>
                        {entry.currency !== "AED" && entry.amount && <div style={{ fontSize:11, color:V.faint }}>≈ AED {aed.toFixed(0)}</div>}
                        <div style={{ fontSize:11, fontWeight:700, padding:"2px 8px", borderRadius:999, display:"inline-block", marginTop:3, background:isPaid?"rgba(22,163,74,0.12)":"rgba(239,68,68,0.08)", color:isPaid?"#16a34a":"#ef4444" }}>
                          {isPaid?"Paid":"Pending"}
                        </div>
                      </div>
          ...
          <div style={{ background:V.card, border:`1px solid ${V.border}`, borderRadius:18, width:"min(460px,100%)" }} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{ padding:20, display:"flex", flexDirection:"column", gap:14 }}>
              <label style={{ display:"flex", flexDirection:"column", gap:5, fontSize:12, fontWeight:700, color:V.muted, textTransform:"uppercase", letterSpacing:"0.06em" }}>
                Month
                <input type="month" style={inp} value={newMonth} onChange={e=>setNewMonth(e.target.value)} />
              </label>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 80px", gap:8 }}>
                <label style={{ display:"flex", flexDirection:"column", gap:5, fontSize:12, fontWeight:700, color:V.muted, textTransform:"uppercase", letterSpacing:"0.06em" }}>
                  Amount
                  <input type="number" style={inp} value={newAmount} onChange={e=>setNewAmount(e.target.value)} placeholder={`Default: ${item.defaultAmount??0}`} />
                </label>
                <label style={{ display:"flex", flexDirection:"column", gap:5, fontSize:12, fontWeight:700, color:V.muted, textTransform:"uppercase", letterSpacing:"0.06em" }}>
                  Cur
                  <select style={inp} value={newCurrency} onChange={e=>setNewCurrency(e.target.value as Currency)}>
                    <option>AED</option><option>INR</option><option>USD</option>
                  </select>
                </label>
              </div>
              <label style={{ display:"flex", flexDirection:"column", gap:5, fontSize:12, fontWeight:700, color:V.muted, textTransform:"uppercase", letterSpacing:"0.06em" }}>
                Note (optional)
                <input style={inp} value={newNote} onChange={e=>setNewNote(e.target.value)} />
              </label>
              {missingMonths.length > 0 && (
                <div style={{ fontSize:12, color:V.faint }}>
                  Missing: {missingMonths.slice(0,6).map(m => (
                    <button key={m} onClick={() => setNewMonth(m)} style={{ ...btn, padding:"2px 8px", fontSize:11, marginLeft:4, color:newMonth===m?V.accent:V.muted }}>
                      {m}
                    </button>
                  ))}
                </div>
            ...
            <div style={{ padding:"0 20px 20px", display:"flex", justifyContent:"flex-end", gap:8 }}>
              ...
              <button style={btnPrimary} onClick={addMissingMonth}>Save</button>
            </div>
          </div>
        </div>
      )}
