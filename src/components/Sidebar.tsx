      ...
      {open && (
        <div className="absolute bottom-full left-0 mb-2 w-full min-w-[220px] rounded-xl border border-sidebar-border bg-sidebar shadow-2xl overflow-hidden z-[80]">
          <Link
            ...
            className="flex items-center gap-3 px-4 py-3 text-sm text-sidebar-text hover:text-white hover:bg-sidebar-hover transition-colors"
          >
            <GearIcon />
            Settings
          </Link>
          ...
          {mode === "export" ? (
            <>
              ...
              <div className="mb-3 flex items-center justify-between">
                <p className="text-sidebar-text text-xs">
                  {format === "json"
                    ? "Includes profile settings and portfolio alerts. Good. A backup that actually backs things up."
                    : "CSV export for spreadsheet use. Properly escaped, unlike many things in life."}
                </p>
              </div>
            ...
            <>
              <p className="text-sidebar-text text-xs mb-4">
                Restore from a MyLife JSON backup. This uses upsert, so matching IDs
    ...
    return (
      ...
      >
        ...
        <div className="border-t border-sidebar-border px-2 py-3 flex-shrink-0">
          <Tooltip label="Export / Restore" show={collapsed}>
            <button
              onClick={() => setShowBackup(true)}
              aria-label="Export or restore data"
              className={clsx(
                "w-full flex items-center gap-3 rounded-lg text-sm text-sidebar-text hover:text-white hover:bg-sidebar-hover transition-all mb-1",
                collapsed ? "px-2 py-2.5 justify-center" : "px-3 py-2.5"
              )}
            >
              <DownloadIcon />
              {!collapsed && <span className="font-medium">Export / Restore</span>}
            </button>
          </Tooltip>
          ...
          <Tooltip label="Settings" show={collapsed}>
            <Link
              ...
              className={clsx(
                "flex items-center gap-3 rounded-lg text-sm transition-all duration-150 mb-1",
                collapsed ? "px-2 py-2.5 justify-center" : "px-3 py-2.5",
          ...
          {!collapsed ? (
            ...
            <div className="flex flex-col gap-1">
              ...
              <Tooltip label="Account menu" show>
                <button
                  onClick={() => setShowBackup(true)}
