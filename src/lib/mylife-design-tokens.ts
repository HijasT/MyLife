 ...
 */
export function ExampleCard({ isDark = false }: { isDark?: boolean }) {
  const colors = getMyLifeColors(isDark);
  ...
  return (
    <div
      style={{
        ...
        borderRadius: mylifeBorderRadius.lg,
        border: `1px solid ${colors.border}`,
        boxShadow: isDark ? mylifeShadows.dark.md : mylifeShadows.light.md,
      }}
    >
      <h3
        style={{
          fontSize: mylifeTypography.fontSize.xl,
      ...
      <p
        style={{
          fontSize: mylifeTypography.fontSize.base,
          color: colors.textMuted,
        }}
      >
        This is an example using the MyLife design tokens.
      </p>
      <button
        style={{
          background: mylifeColors.primary,
          color: mylifeColors.onPrimary,
          padding: `${mylifeSpacing[3]} ${mylifeSpacing[6]}`,
          borderRadius: mylifeBorderRadius.lg,
          fontWeight: mylifeTypography.fontWeight.semibold,
          border: 'none',
          cursor: 'pointer',
          marginTop: mylifeSpacing[4],
          transition: `background ${mylifeTransitions.normal}`,
        }}
        onMouseEnter={(e) => {
          e.currentTarget.style.background = mylifeColors.primaryHover;
        }}
        onMouseLeave={(e) => {
          e.currentTarget.style.background = mylifeColors.primary;
        }}
      >
        Primary Button
      </button>
